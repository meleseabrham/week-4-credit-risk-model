...
with st.form("risk_form"):
    ...
    with col1:
        ...
        val = st.number_input("Transaction Value", min_value=0.0, value=1000.0, step=100.0)
        provider_id = st.text_input("Provider ID", value="Provider_1")
        channel_id = st.selectbox("Channel ID", ["Web", "Android", "iOS", "PayLater"])
        
    with col2:
        product_id = st.text_input("Product ID", value="Product_1")
        product_category = st.selectbox("Product Category", ["Financial Services", "Airtime", "DataBundles", "UtilityBill"])
...
if submit_button:
    ...
    with st.spinner("Analyzing Risk..."):
        try:
            ...
            # If running locally outside docker, user might need to change this or use localhost.
            # We'll try-catch for better UX.
            ...
            st.error("Failed to connect to the scoring API. Ensure the API service is running.")
